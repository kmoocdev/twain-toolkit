...
LRESULT FAR PASCAL DO_PM_XFERDONE(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam);

BOOL __stdcall CALL_EjectCtl_MSG_SET(BOOL EjectInstruct); //ÛÉõóò¦ïÒ
extern LRESULT FAR (PASCAL * CallBackEjectProc)(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam);
// extern LRESULT FAR (PASCAL * CallBackStopProc)(HWND hWnd,UINT iMessage,WPARAM wParam,LPARAM lParam);
extern BOOL gbStop;
extern TW_UINT16 gtwRC2;

extern HGLOBAL _ghDIB; // hDib to the current DIB
...
//		Show -
//		Flag - 
//  RETURNS: TWRC_SUCCESS is successful
// 
...
// 		if (TWOpenDS() == TRUE)
// 		{
			if (TWXferMech (hWnd, TWSX_NATIVE) == TWRC_SUCCESS)
			{
				if (!TWIsDSEnabled())
				{
...
BOOL ProcessTWMessage(LPMSG lpMsg, HWND hWnd)
{
	...
	TW_UINT16  twRC = TWRC_NOTDSEVENT;	// #define TWRC_NOTDSEVEVT  5 
	TW_EVENT   twEvent;
	...
	if ((TWIsDSMOpen()) && (TWIsDSOpen()))
	{
		...
		{
			case MSG_XFERREADY:
				OutputDebugString("ProcessTWMessage: MSG_XFERREADY");
				//If AcqFlag >0 then we are in state 5
				if (AcqFlag)
				{
// 					MessageBox(NULL,"ProcessTWMessage", "a",MB_OK);
			...
			case MSG_CLOSEDSREQ:
			case MSG_CLOSEDSOK:
				OutputDebugString("ProcessTWMessage: MSG_CLOSEDSREQ, MSG_CLOSEDSOK");
				/*
				...
				*/
				...
				if(TWDisableDS())
					if(TWCloseDS())
						TWCloseDSM(NULL);
				(*CallBackEjectProc)(hWnd, 0, 0, TWCRC_HOPPER_EMPTY);
				break;
		...
		// possible new message
			case MSG_NULL:
			default:
				OutputDebugString("ProcessTWMessage: MSG_NULL, default");
				break;
		}   
	} 
...
{ 
	...
	if (OpenFile("c:\\TWCall.log",&of,OF_EXIST)==HFILE_ERROR)
	{
		file = OpenFile("c:\\TWCall.log",&of,OF_CREATE|OF_WRITE|OF_SHARE_DENY_NONE);
	}
	else
	{
		file = OpenFile("c:\\TWCall.log",&of,OF_WRITE|OF_SHARE_DENY_NONE);
	}
