...
{
// 	FARPROC proc = NULL;
	int (__stdcall *proc)(struct HWND__ *,unsigned int,unsigned int,long);
	...
	else
	{
		...
		else
		{
			...
			*/
			if (hStruct = GlobalAlloc(GHND, 2048))
			{
				pStruct = (char*)GlobalLock(hStruct);

				/*
				* send structure command and show results 
				*/
				if (proc = MakeProcInstance(SendDlgProc, (HINSTANCE)hInst))
				{
					if (DialogBoxParam((HINSTANCE)hInst, MAKEINTRESOURCE(DLG_SEND), (HWND)hWnd, proc, 0L) == -1)
					{
						if (MessageLevel()  >= ML_ERROR)
						{
							ShowRC_CC(hWnd, 0, 0, 0,
				...
				*/
				if (pStruct) 
				{ 
					GlobalUnlock(hStruct); pStruct = NULL;
				}
			...
			*/
				 ...
				 (pItemTypeEntry->ItemId == pTableCap->ExpectedType))
			{
				g_hComboBox = CreateWindow("Combobox", "", 
										...
										hDlg, (HMENU)IDCOMBOBOX, (HINSTANCE)hInst, NULL); 
			...
			else
			{
				...
				*/
				if(pTableCap->ExpectedType != TWTY_FRAME)
				{
					g_hEditBox = CreateWindow("edit", "", 
											...
											hDlg,	(HMENU)IDEDITBOX, (HINSTANCE)hInst, NULL);
				...
				*/
				else
				{
					...
					*/
					Change = DialogBox((HINSTANCE)hInst, MAKEINTRESOURCE(DLG_FRAME), hDlg, FrameDlgProc);
	...
	switch(dat)
	{
		...
		*/
		case DAT_IMAGEFILEXFER:
			if ((ret = ImageFileXfer((void*)bmp)) == TWRC_SUCCESS)
			{
...
typedef	long (__stdcall *FunctionTypedef)(struct HWND__ *,unsigned int,unsigned int,long);

LRESULT CALLBACK SubClassEditProc(HWND hWnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	LRESULT  result = 0;

	WNDPROC wp = (FunctionTypedef)GetProp(hWnd, g_szSubClassEditBoxProc);
