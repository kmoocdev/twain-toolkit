...
/* Scanner Button Click Event : 2005/07/06 - Jeon Mira */
/*
typedef struct {
	TW_UINT16	SwStatus;
	TW_UINT16	Reserved;
...
typedef struct {
	TW_UINT8	bLength;
	TW_UINT8	bDescriptor;
	TW_UINT16	wTotalLength;
	TW_UINT8	bNumInterface;
	TW_UINT8	bConfigurationValue;
	TW_UINT8	iConfiguration;
	TW_UINT8	bmAttributes;
	TW_UINT8	MaxPower;
} TW_USB_CONFIGURATION, FAR *pTW_USB_CONFIGURATION;

typedef struct {
	TW_UINT8	bLength;
	TW_UINT8	bDescriptor;
	TW_UINT8	bEndpointAddress;
	TW_UINT8	bmAttributes;
	TW_UINT16	wMaxPacketSize;
	TW_UINT8	bInterval;
} TW_USB_ENDPOINT, FAR *pTW_USB_ENDPOINT;

typedef struct {
	TW_UINT8	bLength;
	TW_UINT8	bDescriptor;
	TW_UINT8	bInterfaceNumber;
	TW_UINT8	bAlternateSetting;
	TW_UINT8	bNumEndpoints;
	TW_UINT8	bInterfaceClass;
	TW_UINT8	bInterfaceSubClass;
	TW_UINT8	bInterfaceProtocol;
	TW_UINT8	iInterface;
} TW_USB_INTERFACE, FAR *pTW_USB_INTERFACE;

typedef struct {
	TW_USB_CONFIGURATION	cd;
	TW_USB_INTERFACE		id;
	TW_USB_ENDPOINT			ed[2];
	TW_UINT16				Reserved;
} TW_USBINFO, FAR *pTW_USBINFO;
*/
/* No DAT needed. */
typedef struct {
   TW_FIX32   X;
...
/* Hitachi OverHead Scanner - Button Control : 2005/07/06 - Jeon Mira */
//#define DAT_SW				0x8001 /* Scanner Switch */
//#define DAT_USBINFO			0x8019
...
// 웹 검색 추가
#define ICAP_FEEDERTYPE					0x1154	// http://guides.gdpicture.com/v6/html/content/frames.html?frmname=topic&frmfile=TwainCapabilities.html
