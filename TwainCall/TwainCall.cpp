...
extern "C" {
	extern  LRESULT FAR(PASCAL* PrevProc)(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam);
	extern  LRESULT FAR(PASCAL* CallBackProc)(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam);
	extern  LRESULT FAR(PASCAL* CallBackEjectProc)(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam);
}
	...
	FREE_IMAGE_FORMAT fiFormat, int optFlag)
{
	...
	if (TWAcquire(hWnd, show, Flag) == TRUE)
	{
...
{
	LogMessage("CALL_TWAppAquireFixedName\r\n");
	TW_INT16 Flag;
	HWND hWnd = hMainWnd;
	int show;
...
int __stdcall Call_ProcessTWMessage(LPMSG lpMsg, HWND hWnd)
{
	...
	LogMessage("Call_ProcessTWMessage\r\n");
...
int __stdcall CALL_TWAppQuit()
{
	...
	LogMessage("CALL_TWAppQuit\r\n");
	if (CloseApplication(hMainWnd))
		CleanKillApp();

	SetWindowLong(hMainWnd, GWLP_WNDPROC, (long)PrevProc);
	PrevProc = 0;
...
{
	LogMessage("CALL_SetProperty_Frame\r\n");
...
{
	LogMessage("CALL_SetProperty_String\r\n");
	TW_UINT16 conType, itemType;
	int rc;
...
{
	LogMessage("CALL_SetProperty_Array\r\n");
	int rc;
	TW_UINT16 conType;
	TW_UINT16 itemType;
	...
	BOOL EjectInstruct) //排出指定
{
	...
	char ErrMsg[512]; //エラ?メッセ?ジ
	TW_UINT16 ConditionCode = 999; //コンディションコ?ド
	static TW_EJECTCTL twEjectCtl; //排出制御情報
